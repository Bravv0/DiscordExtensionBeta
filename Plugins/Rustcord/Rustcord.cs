...
{
    [Info("Rustcord", "Kirollos & OuTSMoKE", "3.0.0")]
    ...
    internal class Rustcord : RustPlugin
    {
        ...
        private class Settings
        {
            ...
            private Snowflake _Botid { get; set; }
            public Snowflake Botid(Snowflake id = default(Snowflake))
            {
                if (id.IsValid())
                {
                    _Botid = id;
        ...
        private void OnDiscordClientCreated()
        {
            ...
            try
            {
                _client.Connect(_settings.Apikey, GatewayIntents.Guilds | GatewayIntents.GuildMembers | GatewayIntents.GuildMessages | GatewayIntents.DirectMessages);
            }
            catch (Exception e)
            {
                flag = false;
        ...
        {
            Puts($"{player?.displayName} | {player?.currentTeam} | {team} | {team?.GetLeader()?.displayName} | {team?.teamID.ToString()}");
